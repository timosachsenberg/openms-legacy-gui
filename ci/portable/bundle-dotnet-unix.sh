#!/usr/bin/env bash
# Stage the .NET 8 runtime and hostfxr app-locally next to the GUI package so the
...
if [[ "$RUNNER_OS" == "macOS" ]]; then
  ...
  # rewritten the application dependency graphs. Otherwise it treats every
  # executable bit in the .NET runtime as another app executable.
  target_dotnet_root="$GUI_STAGE/.dotnet-runtime"
else
  target_dotnet_root="$GUI_STAGE/dotnet"
fi
mkdir -p \
