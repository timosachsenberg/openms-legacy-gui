...
namespace OpenMS
{
  ...
  std::string QApplicationTOPP::configurePortableEnvironment()
  {
    ...
    QDir package_root(executable_dir);
    // <package>/bin/<tool>, including a macOS command-line symlink.
    package_root.cdUp();

    QString data_path = package_root.filePath("share/OpenMS");
#ifdef __APPLE__
    if (!QFileInfo(QDir(data_path).filePath("CHEMISTRY/unimod.xml")).isFile())
    {
      // <package>/<tool>.app/Contents/MacOS/<tool>
      package_root = executable_dir;
      package_root.cdUp();
      package_root.cdUp();
      package_root.cdUp();
      data_path = package_root.filePath("share/OpenMS");
    }
#endif
